...
const getVoterNum = (poll_id) => {
  return db.query(`SELECT count(*) AS num_voters, sum(rank) AS sum_rank
...
const getPollResults = (poll_id) => {
  return db.query(`SELECT polls.id AS poll_id, options.id AS option_id, options.title, polls.name, sum(rank) AS scores
    FROM polls JOIN options ON polls.id = poll_id
...
const getUserNameEmailUserid = (poll_id) => {
  return db.query(`SELECT users.name AS user_name, users.email AS user_email, users.id AS user_id
