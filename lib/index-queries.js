...
const postIndex = (name, id, optionsArr, descriptionsArr) => {
  return db.query('INSERT INTO polls (name, owner_id) VALUES ($1, $2) RETURNING *', [name, id])
    ...
    .then((response) => {
      ...
      // Pass in each option and description from their
      // respective arrays. Each option and description
      // will be in the same position in their respective arrs.
      for (let i = 0; i < optionsArr.length; i++) {
        const option = optionsArr[i];
        console.log("option", option);
        const description = descriptionsArr[i];
        console.log("description", description);
        db.query('INSERT INTO options (title, description, poll_id) VALUES ($1, $2, $3) RETURNING *', [option, description, optionId]);
      }
      return;
    });
};
